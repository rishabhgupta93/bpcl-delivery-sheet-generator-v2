...
def test_enrich_exact_consumer_and_invoice_match_populates_enrichment_fields() -> None:
    ...
    assert result.summary.matched_records == 1
    assert result.summary.unmatched_records == 0
    assert result.summary.online_payment_count == 1
    assert result.warnings == []
...
def test_enrich_accepts_online_payment_with_net_amount() -> None:
    records = [_delivery_record(consumer_number="96076255", cash_memo_no="29218")]
    extraction_records = [
        _extraction_record(
            consumer_number="96076255",
            invoice_no="29218",
            online_payment="Y/448.50",
        )
    ]
    ...
    assert result.records[0].online_payment == "Y/448.50"
    assert result.audit_rows[0]["online_payment"] == "Y/448.50"
    assert result.summary.online_payment_count == 1
    assert result.warnings == []


def test_enrich_accepts_online_payment_with_zero_net_amount() -> None:
    records = [_delivery_record(consumer_number="96076255", cash_memo_no="29218")]
    extraction_records = [
        _extraction_record(
            consumer_number="96076255",
            invoice_no="29218",
            online_payment="Y/0.00",
        )
    ]
    ...
    assert result.records[0].online_payment == "Y/0.00"
    assert result.summary.online_payment_count == 1
    assert result.warnings == []


def test_enrich_invalid_online_payment_value_raises_error() -> None:
    records = [_delivery_record(consumer_number="96076255", cash_memo_no="29218")]
    extraction_records = [
        _extraction_record(
            consumer_number="96076255",
            invoice_no="29218",
            online_payment="448.50",
        )
    ]
