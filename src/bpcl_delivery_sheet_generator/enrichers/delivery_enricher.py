...
class DeliveryEnricher:
    ...
    def _build_summary(
        ...
        return ExtractionSummary(
            ...
            mandatory_inspection_due_count=sum(
                ...
                if record.mandatory_inspection_due == "Y"
            ),
            biometric_due_count=sum(
                1 for record in extraction_records if record.biometric_due == "Y"
            ),
            ...
            online_payment_count=sum(
                1
                for record in extraction_records
                if self._is_online_payment(record.online_payment)
            ),
        )
    ...
    def _validate_extraction_record(
        ...
        invalid_due_values = {
            field: value
            for field, value in due_values.items()
            if value not in self._VALID_ENRICHMENT_VALUES
        }

        if invalid_due_values:
            raise DeliveryEnricherError(
                f"Invalid due enrichment values detected for "
                f"consumer_number={extraction.consumer_number}, "
                f"invoice_no={extraction.invoice_no}: {invalid_due_values}"
            )

        if not self._is_valid_online_payment_value(extraction.online_payment):
            raise DeliveryEnricherError(
                f"Invalid online_payment value detected for "
                f"consumer_number={extraction.consumer_number}, "
                f"invoice_no={extraction.invoice_no}: "
                f"{extraction.online_payment}"
            )
    ...
    def _is_valid_online_payment_value(self, value: str) -> bool:
        if value in self._VALID_ENRICHMENT_VALUES:
            return True

        return bool(re.fullmatch(r"Y/[0-9]+(?:\.[0-9]{1,2})?", value))

    def _is_online_payment(self, value: str) -> bool:
        return value == "Y" or value.startswith("Y/")
